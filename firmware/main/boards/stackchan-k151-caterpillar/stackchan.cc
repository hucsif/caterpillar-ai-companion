...
public:
    ...
    TouchState ReadTouchState() {
        ...
        s.ok = true;
        // 只接受 H(0x3) 级别触摸，过滤 L/M 噪声误触发
        s.zone[0] = ((s.output1_raw >> 0) & 0x3) == 0x3;  // CH1
        s.zone[1] = ((s.output1_raw >> 2) & 0x3) == 0x3;  // CH2
        s.zone[2] = ((s.output1_raw >> 4) & 0x3) == 0x3;  // CH3
        return s;
    }
...
private:
    ...
    static constexpr uint32_t VECTOR_AVATAR_UPDATE_MS = 30;
    bool emoji_mode_ = false;  // false=矢量表情(默认), true=小黄脸emoji
    ...
    static constexpr int BOOT_INIT_PITCH_DEG = 20;
    ...
    static stackchan::avatar::Emotion MapEmotionToAvatar(const char* emotion) {
        ...
        if (strcmp(emotion, "surprised") == 0 || strcmp(emotion, "shocked") == 0)
            return stackchan::avatar::Emotion::Happy;    // 惊讶→睁大眼
        if (strcmp(emotion, "embarrassed") == 0)
            return stackchan::avatar::Emotion::Sad;       // 害羞→垂眼
        return stackchan::avatar::Emotion::Neutral;
    }

    void SyncEmotionToVectorAvatar(const char* emotion) {
        if (!vector_avatar_) {
            ESP_LOGW(TAG, "SyncEmotionToVectorAvatar('%s') SKIPPED: vector_avatar_ is null", emotion);
            return;
        }
        if (!display_) {
            ESP_LOGW(TAG, "SyncEmotionToVectorAvatar('%s') SKIPPED: display_ is null", emotion);
            return;
        }
        auto e = MapEmotionToAvatar(emotion);
        ESP_LOGI(TAG, "SyncEmotionToVectorAvatar('%s') → emotion=%d", emotion, (int)e);
        DisplayLockGuard lock(display_);
    ...
    void SwitchToVectorMode() {
        ...
        if (display_) {
            // emoji UI 已屏蔽，不需要隐藏
            display_->BringUIBarsToFront();
        }
        if (vector_avatar_ && vector_avatar_->getPanel()) {
            DisplayLockGuard lock(display_);
            lv_obj_remove_flag(vector_avatar_->getPanel()->get(), LV_OBJ_FLAG_HIDDEN);
        }
        ESP_LOGI(TAG, "Avatar mode switched to: VECTOR");
    }
    ...
    void SwitchToEmojiMode() {
        ...
        if (vector_avatar_ && vector_avatar_->getPanel()) {
            DisplayLockGuard lock(display_);
            lv_obj_add_flag(vector_avatar_->getPanel()->get(), LV_OBJ_FLAG_HIDDEN);
        }
        ESP_LOGI(TAG, "Avatar mode switched to: EMOJI");
    }
    ...
    static void VectorAvatarUpdateCb(void* arg) {
        ...
        if (++frame_count % 10 == 0) {
            if (self->emoji_mode_) {
                if (auto* panel = self->vector_avatar_->getPanel()) {
                    lv_obj_add_flag(panel->get(), LV_OBJ_FLAG_HIDDEN);
                }
                self->display_->BringUIBarsToFront();
            } else {
                if (auto* panel = self->vector_avatar_->getPanel()) {
                    lv_obj_remove_flag(panel->get(), LV_OBJ_FLAG_HIDDEN);
                }
                self->display_->BringUIBarsToFront();
            }
        }
    ...
    bool TryCreateVectorAvatar() {
        if (vector_avatar_) return true;
        if (!display_) return false;
        DisplayLockGuard lock(display_);
        lv_obj_t* screen = lv_screen_active();
        if (!screen) return false;
        if (lv_obj_get_child_cnt(screen) == 0) return false;
        ...
        vector_avatar_->init(screen, LV_FONT_DEFAULT);
        ...
        esp_timer_create_args_t update_args = {
            .callback = VectorAvatarUpdateCb, .arg = this,
            .dispatch_method = ESP_TIMER_TASK, .name = "vec_av_upd",
            .skip_unhandled_events = true,
        };
        ...
        // 默认模式：矢量表情（隐藏 emoji）
        // emoji removed
        ESP_LOGI(TAG, "Vector avatar ready: blink=%d breath=%d update=%" PRIu32 "ms (emoji hidden)",
    ...
    void RegisterMcpTools() {
        ...
        {
            PropertyList props;
            props.AddProperty(Property("mode", kPropertyTypeString, std::string("vector")));
            mcp_server.AddTool(
                "self.display.switch_avatar_mode",
                "Switch between vector avatar and emoji/GIF display. "
                "mode: 'vector' (default) or 'emoji'.",
                props,
                [this](const PropertyList& properties) -> ReturnValue {
                    ...
                    if (mode == "emoji") {
                        SwitchToEmojiMode();
                        cJSON_AddStringToObject(root, "mode", "emoji");
                    } else if (mode == "vector") {
                        SwitchToVectorMode();
                        cJSON_AddStringToObject(root, "mode", "vector");
                    } else {
                        SwitchToVectorMode();
                        cJSON_AddStringToObject(root, "mode", "vector");
                    }
        ...
        caterpillar_blufi_register_mcp_tools(mcp_server);
...
public:
    StackChanBoard() {
        ...
        // 夜间模式回调（硬件到货后实现背光/舵机/LED 控制）
        caterpillar_set_night_mode_callback([this](bool night) {
            if (night) {
                GetBacklight()->SetBrightness(0);  // 熄屏
            } else {
                GetBacklight()->RestoreBrightness();
            }
        });

        // LD2450 转头追踪：10fps实时追踪，速度拉满
        caterpillar_set_head_angle_callback([this](int16_t yaw, int16_t pitch) {
            // R60ABD1 3D追踪：同时支持水平(yaw)+垂直(pitch)
            if (!servo_ok_ || motion_driver_ == nullptr) return;
            if (!TakeMotionMutexAfterTorqueEngaged()) return;
            if (servo_wobble_active_.load()) {
                servo_wobble_active_.store(false);
                servo_wobble_step_.store(0);
            }
            motion_driver_->StartMove(yaw, pitch, 40, true);
            xSemaphoreGive(motion_mutex_);
        });

        // caterpillar_http_init(nullptr);  // MQTT 广播 — 已禁用
        caterpillar_radar_init();        // 毫米波雷达 — 已禁用
        caterpillar_blufi_init();   // 无WiFi时自动启动BLE配网
        RegisterMcpTools();         // MCP工具注册
    }
    ...
    // ---- stackchan::Modifiable 接口 ----
    stackchan::avatar::Avatar& avatar() override { return *vector_avatar_; }
    bool hasAvatar() override { return vector_avatar_ != nullptr; }
    ...
    virtual void OnTtsStart() override {
        ...
        // 添加说话修饰器：嘴巴随机开合模拟说话
        if (vector_avatar_) {
            speaking_modifier_id_ = avatar_modifier_pool_.create(
                std::make_unique<stackchan::SpeakingModifier>(0, 180));
            ESP_LOGD(TAG, "SpeakingModifier added (id=%d)", speaking_modifier_id_);
        }
    }
    ...
    virtual void OnTtsStop() override {
        ...
        if (speaking_modifier_id_ >= 0) {
            avatar_modifier_pool_.destroy(speaking_modifier_id_);
            speaking_modifier_id_ = -1;
            ESP_LOGD(TAG, "SpeakingModifier removed");
        }
    }
    ...
    // 唤醒词触发 → 看向人（一次性，R60ABD1 DP5 驱动 yaw+pitch）
    virtual void OnWakeWord() override {
        int16_t yaw = caterpillar_get_nearest_person_yaw();
        if (yaw != INT16_MIN) {
            ESP_LOGI(TAG, "Wake word: turning to person yaw=%d", (int)yaw);
            WriteHeadAngles(yaw, BOOT_INIT_PITCH_DEG, 240);
        }
    }
